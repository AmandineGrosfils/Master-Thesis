...
save_path = "cross-validation_folds_MinMaxMaxMin_Train/"
...
def is_twoCorners(PAIR_12_CELL, entrie, doses1, doses2):

    if PAIR_12_CELL.iloc[i]["Conc1"]==doses1[0] and  PAIR_12_CELL.iloc[i]["Conc2"]==doses2[2]:
        twoCorners = 1
    elif PAIR_12_CELL.iloc[i]["Conc1"]==doses1[2] and  PAIR_12_CELL.iloc[i]["Conc2"]==doses2[0]:
        twoCorners = 1
    else:
        twoCorners = 0
...
for i in range(9):
    twoCorners = is_twoCorners(PAIR1, i, d1, d2)
    if twoCorners == 0:
...
for i in range(9):
    twoCorners = is_twoCorners(PAIR2, i, d1, d2)
    if twoCorners == 0:
        write_in_file(get_index_pair12[i], name_test)
    else : 
        write_in_file(get_index_pair12[i], name_train)
...
for i in range(9):
    twoCorners = is_twoCorners(PAIR3, i, d1, d2)
    if twoCorners == 0:
        write_in_file(get_index_pair12[i], name_test)
    else : 
        write_in_file(get_index_pair12[i], name_train)
    ...
    for d2 in all_drugs_2:
        ...
        for c in cells:
            ...
            for i in range(9):
                ...
                twoCorners = is_twoCorners(PAIR_12_CELL, i, doses1, doses2)
                if twoCorners == 0:
