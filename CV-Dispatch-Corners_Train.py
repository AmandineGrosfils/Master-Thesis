...
save_path = "cross-validation_folds_Corner_Train/"
...
def is_corner(PAIR_12_CELL, entrie, doses1, doses2):
    ...
    if PAIR_12_CELL.iloc[i]["Conc1"]==doses1[0] and  PAIR_12_CELL.iloc[i]["Conc2"]==doses2[0]:
        ...
        corner = 1
    elif PAIR_12_CELL.iloc[i]["Conc1"]==doses1[0] and  PAIR_12_CELL.iloc[i]["Conc2"]==doses2[2]:
        corner = 1
    else:
        corner = 0
...
for i in range(9):
    corner = is_corner(PAIR1, i, d1, d2)
    if corner == 0:
...
for i in range(9):
    corner = is_corner(PAIR2, i, d1, d2)
    if corner == 0:
        write_in_file(get_index_pair12[i], name_test)
    else : 
        write_in_file(get_index_pair12[i], name_train)
...
for i in range(9):
    corner = is_corner(PAIR3, i, d1, d2)
    if corner == 0:
        write_in_file(get_index_pair12[i], name_test)
    else : 
        write_in_file(get_index_pair12[i], name_train)
    ...
    for d2 in all_drugs_2:
        ...
        for c in cells:
            ...
            for i in range(9):
                ...
                corner = is_corner(PAIR_12_CELL, i, doses1, doses2)
                if corner == 0:
