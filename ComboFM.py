...
def main(argv):
    ...
    nfolds_outer = 1 # Number of folds in the outer loop : do a CV and not a nested CV
    ...
    # Read responses
    y  = np.loadtxt("responses.csv", delimiter = ",", skiprows = 1)
    y = np.concatenate((y, y), axis=0)
    
    inner_folds = list(range(1, nfolds_inner+1))

    te_idx = np.loadtxt(data_dir+'Test.txt')
    tr_idx = np.loadtxt(data_dir+'Train.txt')
    ...
    for reg_i in range(len(regparams)):
        ...
        for inner_fold in inner_folds:
            ...
            te_idx_CV = np.loadtxt(data_dir+'Test_CV-%d.txt'%(inner_fold)).astype(int)
            tr_idx_CV = np.loadtxt(data_dir+'Train_CV-%d.txt'%(inner_fold)).astype(int)
    ...
    np.savetxt('results/reg_CV_avg_RPearson.txt', CV_avg_reg)
    ...
    np.savetxt('results/rank_CV_avg_RPearson.txt', CV_avg_rank)
    ...
    print('FINAL MODEL : rank = {}, reg = {}'.format(rank, reg))
    ...
    np.savetxt("results/y_test_order-%d_rank-%d_reg-%d_%s.txt"%(order, rank, reg, experiment), y_te)
    np.savetxt("results/y_pred_order-%d_rank-%d_reg-%d_%s.txt"%(order, rank, reg, experiment), y_pred_te)

    # Save model weights
    weights = model.weights
    for i in range(order):
        np.savetxt('results/P_order%d_rank-%d_reg-%.1e.txt'%(i+1, rank, reg), weights[i])
